function [R,y,l,u,p,z0,beta] = ubils_reduction(B,y,l,u,cp)
%
% [R,y,l,u,p,beta] = ubils_reduction(B,y,l,u) reduces an underdetremined 
% problem or rank-deficient overdetermiend problem to an upper trapezoid 
% form by the QR factorization with column permutations Q'*B*P = [R; 0]. 
...
%         cp = 0 (default), perform simple column permutations;
%         cp = 1, perform sophasitated column permutations, preferable for 
...
%    l - permued lower bound 
%    u - permuted upper bound
%    p - n-dimensional permutation vector representing P
%    beta - initial search radius to be used in the search process

% Subfunctions: radius, reorder (included in this file)
%               obils_reduction (not included in this file)
% 
% Main References: 
...
r = sum(abs(diag(R(1:min(m,n),1:min(m,n)))) > n*eps(abs(R(1,1)))); 
if r == n
...
[beta,z0] = radius(R,y,l,u);

% If beta is 0, z0 is the solution
...
y_m = y(m);
for i = n:-1:m
    if y_m == 0 & R(m,i) == 0 % Avoid NaN
            xr = inf;
    else
            xr = round(y_m/R(m,i));
    end
    x(i) = median([l(i),u(i),xr]);
    y_m = y_m - R(m,i)*x(i);
end
...
% Permute x0 to z0 using the final permutation vector p
z0 = x0(p);
...
% a real solution of the box-constrauined least squares problem. 
%
% Inputs:
