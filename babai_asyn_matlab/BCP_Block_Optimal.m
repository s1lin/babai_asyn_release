function [s_bar_cur, v_norm_cur, stopping] = BCP_Block_Optimal(s_bar_IP, v_norm_IP, H_cur, tolerance, max_Babai, max_Time, y, K, N)

% [s_bar_cur, v_norm_cur, stopping] = BCP_Block_Optimal(s_bar_IP, v_norm_IP, H_cur, tolerance, max_Babai, max_Time, y, K, N)
% applies the BCP-Block Optimal method to obtain a sub-optimal solution
% 
% Inputs:
...
s_bar_cur = s_bar_IP; v_norm_cur = v_norm_IP;
...
while b_count < max_Babai && v_norm_cur > tolerance && toc(timerVal) < max_Time
    ...
    [s_bar_cur, v_norm_cur, b_toadd, indicator, term_boolean] = BCP_opt(H_cur, y, s_bar_cur, v_norm_cur, (max_Babai-b_count), N, indicator, max_Time-toc(timerVal));
...
function [s_bar_cur, v_norm_cur, b_count, indicator, term] = BCP_opt(H, y, s_bar_prev, v_norm_prev, b_rem, N, indicator, max_Time)

% [s_bar_cur, v_norm_cur, b_count, indicator, term] = BCP_opt(H, y, s_bar_prev, v_norm_prev, b_rem, N, indicator, max_Time)
...
%     b_count - integer scalar, number of calls to block_opt.m
%     indicator - 2-by-q integer matrix (indicates submatrices of H)
...
[s_bar_temp, v_norm_temp] = block_opt(H, y, s_bar_prev, N, indicator);
b_count = b_count + 1;
...
for i=1:n_perms
    ...
    [s_bar_temp, v_norm_temp] = block_opt(H, y, s_bar_prev, N, indicator_permuted);
    b_count = b_count + 1;
